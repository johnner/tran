...
const SERVICE_URL = 'http://tran-service.com/api/user/';
...
class Options {
    ...
    return new Promise(function (resolve, reject) {
      this.storage('get', 'auth_token', function (data) {
        if (!data.auth_token) {
          ...
          utils.get(SERVICE_URL, { headers: { 'X-AUTH-TOKEN': data.auth_token } }).then(
            res => resolve(res),
            res => reject(res)
          )
        }
  ...
  signin() {
    ...
    utils.post(SERVICE_URL, { data: cred }).then(
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('memorize').addEventListener('click', () => options.memorize());
  document.getElementById('signinBtn').addEventListener('click', () => options.signin());
